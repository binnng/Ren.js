...
function getClassNameArr(node) {
  return ("" + className(node)).split(/\s+/g);
}
...
Ren.fn =  {
  ...
  dom: function(selector, node) {
     ...
     */
    } else if (selector instanceof Ren.fn.dom) {
...
Ren.fn.dom.prototype = {
  ...
  addClass: function(name){
    ...
    var node = this.context[0];
    var classList = [];
    var cls = className(node);

    if (!Ren.dom(node).hasClass(name)) {
      classList.push(name);
    }
  ...
  hasClass: function(name) {
    var node = this.context[0];
    var classList = getClassNameArr(node);

    return Ren.arr(classList).has(name);
  },
  ...
  removeClass: function(name) {
    ...
    classList = Ren.arr(classList).remove(name);
  ...
  after: function(dom) {
    var node = this.context[0];
    var target = (dom instanceof Ren.fn.dom) ? dom.get(0) : dom;
    var before = nextSibling(node);

    Ren.dom(before).before(target);
...
Ren.getJSON = function(url, callback) {
  Ren.ajax({
    url: url,
    isJSON: true,
    success: callback
